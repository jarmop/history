...
const Scrollbar: React.FC<ScrollbarProps> = ({
  ...
  const [scrollbarContainerWidth, setScrollbarWidth] = useState<
    number | undefined
  >(0)
  const timeSpan = endYear - startYear
  const unitSize = 100
  const years = Array.from(
    { length: timeSpan / unitSize },
    (value, index) => startYear + index * unitSize,
  )
  ...
  return (
    <div
      ...
      style={{ width: scrollbarContainerWidth }}
    >
      <div className={styles.scrollbar} ref={scrollbarDomObject}>
        <div
          ...
          style={{
            ...
            top:
              ((windowTimeSpan.startYear - startYear) / timeSpan) * 100 + '%',
            height:
              ((windowTimeSpan.endYear - windowTimeSpan.startYear) / timeSpan) *
                100 +
              '%',
          }}
        ></div>
